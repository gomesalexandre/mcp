...
func newHyperliquidBuildStakeTool() mcp.Tool {
	return mcp.NewTool("hyperliquid_build_stake",
		mcp.WithDescription(
			"Build an unsigned Hyperliquid cDeposit action to stake HYPE tokens. "+
		...
		mcp.WithString("wei",
			mcp.Description("Amount of HYPE in wei to stake (integer string, e.g. \"1000000000000000000\" for 1 HYPE)."),
			mcp.Required(),
		),
...
func handleHyperliquidBuildStake(store *vault.Store, hlClient *hyperliquid.Client) server.ToolHandlerFunc {
