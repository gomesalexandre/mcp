...
func RegisterAll(s *server.MCPServer, store *vault.Store, pool *evmclient.Pool, cgClient *coingecko.Client, bcClient *blockchair.Client, swapSvc *swap.Service, tcClient *thorchain.Client, mcClient *mayachain.Client, solClient *solanaclient.Client, jupClient *jupiter.Client, xrpClient *xrpclient.Client, tronClient *tronclient.Client, gaiaClient *gaiaclient.Client, pfClient *pumpfunclient.Client, fbClient *fourbyte.Client, vcClient *verifier.Client, dlClient *defillama.Client, hlClient *hyperliquid.Client) error {
	...
	toolmeta.Register(s, newHyperliquidBuildWithdrawTool(), handleHyperliquidBuildWithdraw(store, hlClient), "send", "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildUsdTransferTool(), handleHyperliquidBuildUsdTransfer(store, hlClient), "send", "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildSendAssetTool(), handleHyperliquidBuildSendAsset(store, hlClient), "send", "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildClassTransferTool(), handleHyperliquidBuildClassTransfer(store, hlClient), "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildStakeTool(), handleHyperliquidBuildStake(store, hlClient), "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildUnstakeTool(), handleHyperliquidBuildUnstake(store, hlClient), "hyperliquid")
	toolmeta.Register(s, newHyperliquidBuildDepositTool(), handleHyperliquidBuildDeposit(store, pool), "send", "hyperliquid")
	toolmeta.Register(s, newBridgeAssetTool(), handleBridgeAsset(store, pool), "send", "bridge")
